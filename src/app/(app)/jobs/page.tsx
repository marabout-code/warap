...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    fetchJobs();
  }, [filter, category, search, supabase]);
  ...
  const getEmploymentTypeBadge = (type: string) => {
    ...
    return (
      <span className={`badge ${styles[type] || "badge-neutral"}`}>
        {employmentTypeLabels[type] || type.replace("-", " ")}
      </span>
    );
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Offres de service</h1>
          <p className="mt-1 text-sm text-slate-500">
            Publiez et suivez vos offres de service par catégorie.
          </p>
        </div>
      ...
      {/* Search & filter bar */}
      <div className="card-glass space-y-4 rounded-2xl border-white/60 px-5 py-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Rechercher une offre par service, client ou quartier…"
              value={search}
    ...
    ...categories.map((c) => ({ value: c.id, label: `${c.emoji} ${c.short}` })),
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-primary-200/70 bg-primary-50/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-900">Offres publiques</p>
            <p className="text-xs text-slate-500">
              Les prestataires au Cameroun consultent vos offres sur le site public.
            </p>
          </div>
        </div>
        <Link
        ...
        <div className="overflow-x-auto">
          <table className="card-table w-full">
            ...
            <tbody>
              {jobs.length === 0 ? (
                <tr>
                  <td colSpan={7} className="td text-center py-14">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="mt-4 text-sm font-semibold text-slate-700">Aucune offre trouvée</p>
