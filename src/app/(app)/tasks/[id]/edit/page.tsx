...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    const fetchTask = async () => {
      const [{ data: taskData }, { data: jobsData }, { data: profilesData }] =
        await Promise.all([
      ...
      if (taskData) {
        reset({
          ...
          due_date: taskData.due_date ? taskData.due_date.split("T")[0] : undefined,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        <Link href={`/tasks/${taskId}`} className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-500 shadow-sm transition-all duration-200 hover:border-slate-300 hover:text-slate-700 hover:shadow">
